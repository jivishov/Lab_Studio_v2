...
| — | Hand-control wording (handoff §5.15, "reuses the 2D player's camera help and privacy wording") | **Swap only the names, decided 2026-09-24.** The 2D sentences are kept word for word, except the names of 2D controls Player3D does not have: Camera control → Hand control, Reset → Restart, shelf → tray. The bridge's re-arm prompt follows the same rule in Player3D ("…re-arm hand control."); the 2D player keeps its own text. | `evidence/M7_EVIDENCE.md` |
